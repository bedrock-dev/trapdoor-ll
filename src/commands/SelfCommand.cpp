...
#define ADD_CONFIG(name, hint)                                             \
    ...
    command->mandatory("self", ParamType::Enum, name##Opt,                 \
...
namespace trapdoor {
    const DynamicCommandInstance *setup_selfCommand(int level) {
        ...
        command->mandatory("self", ParamType::Enum, dumpOpt,
                           CommandParameterOption::EnumAutocompleteExpansion);
...
#define ADD_BOOL_CASE(name)                                                                    \
    case do_hash(#name):                                                                       \
        if (bValue.isSet) {                                                                    \
            trapdoor::mod().getUserConfig().SET_FUNC_NAME(name)(playerName, b).sendTo(output); \
        } else {                                                                               \
            trapdoor::mod().getUserConfig().GET_FUNC_NAME(name)(playerName).sendTo(output);    \
        }                                                                                      \
        break;
                // TODO 等有整数型再设置
#define ADD_INT_CASE
