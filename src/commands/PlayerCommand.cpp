        ...
        auto cb = [](DynamicCommand const &command, CommandOrigin const &origin,
            ...
            switch (do_hash(results["player"].getRaw<std::string>().c_str())) {
                ...
                case do_hash("destroypos"):
                    trapdoor::mod()
                        .getSimPlayerManager()
                        .destroyPositionSchedule(name, blockPos, origin.getPlayer(), rep, interval,
                                                 times)
                        .sendTo(output);
                    break;
                ...
                case do_hash("useonpos"):
                    if (results["blockPos"].isSet) {
                        trapdoor::mod()
                            .getSimPlayerManager()
                            .useOnPositionSchedule(name, itemId,
                        ...
                        trapdoor::mod()
                            .getSimPlayerManager()
                            .useOnPositionSchedule(name, itemId, BlockPos(0, 0, 0),
                                                   origin.getPlayer(), rep, interval, times)
                            .sendTo(output);
                    }
                case do_hash("useon"):
                    trapdoor::mod().getSimPlayerManager().useOnSchedule(
                        name, itemId, origin.getPlayer(), rep, interval, times);
                    break;
                case do_hash("cancel"):
                ...
                case do_hash("script"):
                    trapdoor::mod()
                        .getSimPlayerManager()
                        .runScript(name, scriptFile, interval, errorStop)
                        .sendTo(output);
            }
        };
